...
const getProperties = async (req, res) => {
    try {
        ...
        const jsonPathFilters = [];
        if (minBedrooms && parseInt(minBedrooms) > 0) {
            jsonPathFilters.push({ features: { path: ['bedrooms'], gte: parseInt(minBedrooms) } });
        }
        if (minArea && parseInt(minArea) > 0) {
            jsonPathFilters.push({ features: { path: ['area'], gte: parseInt(minArea) } });
        }
        ...
        await (0, redis_1.setCache)(cacheKey, responseData, 300); // 5-minute TTL
...
const getProperty = async (req, res) => {
    try {
        ...
        await (0, redis_1.setCache)(cacheKey, property, 300); // 5-minute TTL
        res.json(property);
    }
    catch (error) {
...
const updatePropertyOrder = async (req, res) => {
    try {
        ...
        await prisma_1.prisma.$transaction(updates.map(({ id, displayOrder }) => prisma_1.prisma.property.update({
            where: { id },
            data: { displayOrder }
        })));
        // Invalidate property list cache
...
exports.updatePropertyOrder = updatePropertyOrder;
