...
const getIndexPath = () => {
    const candidates = [
        path_1.default.join(__dirname, '../../../Look-Immo-Front/dist/index.html'),
        path_1.default.resolve(process.cwd(), '../Look-Immo-Front/dist/index.html'),
        path_1.default.join(__dirname, '../../../Look-Immo-Front/index.html'),
        path_1.default.resolve(process.cwd(), '../Look-Immo-Front/index.html'),
    ];
...
const buildSeoMetaTags = (meta, jsonLd) => {
...
const getIndexHtml = async () => {
    ...
    if (localPath) {
        try {
            return fs_1.default.readFileSync(localPath, 'utf8');
        }
        catch (err) {
            logger_1.logger.error('[SEO Injector] Failed to read local index.html:', err);
        }
    }
    // 2. Try cache
    const now = Date.now();
    ...
    try {
        ...
        logger_1.logger.error('[SEO Injector] Failed to fetch index.html from frontend server:', err);
    }
    return null;
};
// ─── Middleware ────────────────────────────────────────────────────────────────
...
const seoInjector = async (req, res, next) => {
    ...
    if (!template) {
        logger_1.logger.warn('[SEO Injector] Frontend index.html not found — skipping OG injection.');
        next();
        return;
    }
    pruneCache();
...
<article style="padding: 20px; max-width: 800px; margin: 0 auto;">
  ...
  <div style="margin-top: 20px;">
    <h2>Images</h2>
    ${property.images.map((img) => `<img src="${escAttr(resolveImageUrl(img))}" alt="${escAttr(property.title)}" style="max-width: 100%; margin-bottom: 10px; border-radius: 4px;" />`).join('\n')}
  </div>
  ` : ''}
</article>`;
            }
...
<article style="padding: 20px; max-width: 800px; margin: 0 auto;">
  ...
  ${blog.image ? `<img src="${escAttr(resolveImageUrl(blog.image))}" alt="${escAttr(blog.title)}" style="max-width: 100%; margin-top: 20px; border-radius: 4px;" />` : ''}
        ...
        logger_1.logger.error('[SEO Injector] Failed to inject metadata:', err);
        next();
    }
};
exports.seoInjector = seoInjector;
//# sourceMappingURL=seoInjector.js.map