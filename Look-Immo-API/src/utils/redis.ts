...
redisClient.on('error', (err) => {
  if (!hasLoggedError) {
    logger.warn('[REDIS] Could not reach Redis server. Caching disabled. Error:', err.message);
    hasLoggedError = true;
  }
...
export const connectRedis = async (): Promise<void> => {
  try {
    ...
    logger.warn('[REDIS] Could not connect to Redis server. Caching will be disabled. Error:', err);
    isRedisConnected = false;
  }
};
...
export const getCache = async (key: string): Promise<any | null> => {
  ...
  try {
    ...
    logger.warn(`[REDIS] Error getting cache for key ${key}:`, err);
    return null;
  }
};
...
export const setCache = async (key: string, value: any, ttlSeconds: number = 300): Promise<void> => {
  ...
  try {
    ...
    logger.warn(`[REDIS] Error setting cache for key ${key}:`, err);
  }
};
...
export const deleteCache = async (key: string): Promise<void> => {
  ...
  try {
    ...
    logger.warn(`[REDIS] Error deleting cache for key ${key}:`, err);
  }
};
...
export const clearCachePattern = async (pattern: string): Promise<void> => {
  ...
  try {
    ...
    logger.warn(`[REDIS] Error clearing cache pattern ${pattern}:`, err);
  }
};
